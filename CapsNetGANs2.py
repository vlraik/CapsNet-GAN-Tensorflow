...
class CapsConv(object):
	...
	def __call__(self, input, num_outputs, kernel_size=None, stride=None):
		...
		if not self.with_routing:
			...
			capsules = tf.reshape(capsules, (25, -1, self.num_units, 1))

			# [batch_size, 1152, 8, 1]
			capsules = squash(capsules)
			print(capsules.get_shape())
			assert capsules.get_shape() == [25, 1152, 8, 1]
			return(capsules)
			
		else:
			# the DigitCap layer
			# reshape the input into shape [128,1152,8,1]
			input = tf.reshape(input, shape=(25, 1152, 8,1))
			...
			assert capsules.get_shape() == [25,10,16,1]
...
def capsule(input, b_IJ, idx_j):
	...
	with tf.variable_scope('routing'):
		...
		for r_iter in range(3):
			...
			assert s_j.get_shape() == [25, 1, 16, 1]
			...
			assert u_produce_v.get_shape() == [25, 1152, 1, 1]
...
def generator(batch_size, z_dim): 
	...
	with tf.variable_scope('Masking'):
		...
		assert softmax_v.get_shape() == [25, 10, 1, 1]

		# b). pick out the index of max softmax val of the 10 caps
		# [batch_size, 10, 1, 1] => [batch_size] (index)
		argmax_idx = tf.to_int32(tf.argmax(softmax_v, axis=1))
		assert argmax_idx.get_shape() == [25, 1, 1]
		argmax_idx = tf.reshape(argmax_idx, shape=(25, ))
	...
	with tf.variable_scope('Decoder'):
		vector_j = tf.reshape(caps2Gen, shape=(25, -1))
		fc1 = tf.contrib.layers.fully_connected(vector_j, num_outputs=512)
		assert fc1.get_shape() == [25, 512]
		fc2 = tf.contrib.layers.fully_connected(fc1, num_outputs=1024)
		assert fc2.get_shape() == [25, 1024]
...
x_placeholder = tf.placeholder("float", shape=[25, 28,28,1], name='x_placeholder')
...
genimage = tf.reshape(Gz,[-1,28,28])
genimage = tf.expand_dims(genimage,3)
Dg = discriminator(genimage, reuse=True)
...
with tf.variable_scope(scope):    
	d_trainer_fake = tf.train.AdamOptimizer(0.0006).minimize(d_loss_fake, var_list=d_vars) 
	d_trainer_real = tf.train.AdamOptimizer(0.0006).minimize(d_loss_real, var_list=d_vars) 
	g_trainer = tf.train.AdamOptimizer(0.0006).minimize(g_loss, var_list=g_vars)
...
real_images = mnist.validation.next_batch(25)[0].reshape([25, 28, 28, 1])
